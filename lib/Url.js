...
// city url; get url where we replace the city 
// we handle 'localhost' (for development) seperately.
// case 'localhost': add/replace city in querystring
// case 'production': add/replace city in subdomain
Url.replaceCity = function(city, url) {
  url = url || currentUrl();
  var addToUrl = Url.isLocalhost(url) ? Url.addCityToParams : Url.replaceCityInSubdomain;
  var cityUrl = addToUrl(city, url);
  return cityUrl;
}
