...
var filters = {
  ...
  checkDuplicateAccounts: function() {
    this.subscribe('currentUser').wait();
    if(!this.ready()) return;
    if (Meteor.user() && Session.get('requestMergeDuplicateAccount')) {
...
if(Meteor.isClient){
  ...
  Router.before(filters.isLoggedIn, {except: exceptLogin});
  ...
  Router.before(filters.checkDuplicateAccounts, {except: exceptLogin});
  Router.before(filters.allowedAccess, {except: exceptAllowAccess});
...
HackerController = FastRender.RouteController.extend({
  template: 'hacker',
  waitOn: function() {
    return [
  ...
  before: function() { 
    ...
    var hacker = userFromCityRankHash(city, this.params.localRankHash);
...
Router.map(function() {
 ...
 // Loading (for testing purposes)
  ...
  //     else GAnalytics.event('Views', 'profile unknow', this.localRankHash);
  //   },
  //   before: function() { 
